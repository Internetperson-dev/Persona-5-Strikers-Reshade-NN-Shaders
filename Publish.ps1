...
param (
    ...
    ## => User Config <= ## 
    $ProjectPath = "P5S.ReShade.csproj",
    $PackageName = "P5S.ReShade",
    $PublishOutputDir = "Publish/ToUpload",
    $AdditionalFilesDir = "", # Optional: directory whose contents get copied into the package after build.
...
if ($AdditionalFilesDir) { $additionalFilesFullPath = [System.IO.Path]::GetFullPath($AdditionalFilesDir) }
...
if ($AdditionalFilesDir -and (Test-Path $additionalFilesFullPath)) {
    Write-Host "Copying additional files into package"
    Copy-Item -Path "$additionalFilesFullPath/*" -Destination $publishBuildDirectory -Recurse -Force
}
